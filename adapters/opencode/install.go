...
export default {
  id: "agentstatus.opencode.{{.Marker}}",
  server: async (_input) => ({
    event: async ({ event }) => {
      try {
...
func resolvePath(cfg agentstatus.InstallConfig) (string, string) {
	filename := "agentstatus-" + cfg.Marker + ".ts"
	if cfg.Project != "" {
		return filepath.Join(cfg.Project, ".opencode", "plugins", filename), ""
	}
	if cfg.ConfigRoot != "" {
		return filepath.Join(cfg.ConfigRoot, ".config", "opencode", "plugins", filename), ""
	}
...
func installHooks(cfg agentstatus.InstallConfig) (agentstatus.InstallResult, error) {
	...
	if err := tmpl.Execute(&buf, map[string]string{"Endpoint": cfg.Endpoint, "Marker": cfg.Marker}); err != nil {
		return agentstatus.InstallResult{
			Agent:  agentstatus.OpenCode,
...
func uninstallHooks(cfg agentstatus.InstallConfig) (agentstatus.InstallResult, error) {
	...
	return agentstatus.InstallResult{
		Agent: agentstatus.OpenCode, Marker: cfg.Marker,
		Path: path,
	}, nil
}
